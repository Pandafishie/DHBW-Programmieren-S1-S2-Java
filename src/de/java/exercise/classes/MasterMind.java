...
public class MasterMind {
	...
	void rateSchleife() {
		for (versuch = 0; versuch < 50; versuch++) {
			...
			if (richtigeStellen[versuch] == 5) {
				System.out.println("Mit " + (versuch + 1) + " Versuchen gewonnen!");
				break;
			}
		}
		if (versuch == 50) {
		...
		do {
			...
			if (str.length() == 5) {
				for (int i = 0; i < 5; i++) {
					...
					if ((spielerKombination[versuch][i] >= 'a') && (spielerKombination[versuch][i] <= 'h')) {
	...
	void bewertung() {
		...
		for (int k = 0; k <= versuch; k++) {
			...
			richtigeBuchstaben();
			System.out.println(" " + richtigeStellen[k] + " " + falscheStellen[k]);
		}
	}
	...
	void richtigeBuchstaben() {
		...
		for (char ch = 'A'; ch <= 'H'; ch++) {
			...
			falscheStellen[versuch] += Math.min(buchstabeInSpielerKombination, buchstabeInComputerKombination)
					- buchstabeUebereinstimmend;
		}
	}
