...
public class NumberGuess {
	public static void main(String[] args) {
		...
		do {
			...
			do {
				System.out.print("Geben Sie bitte eine Zahl aus dem Bereich " + "1 und 100 ein: ");
				int tip = scan.nextInt();
				trials++;
				if (tip > numberToGuess) {
					System.out.println("Versuch Nr. " + trials + ": " + tip + " zu hoch!");
				} else if (tip < numberToGuess) {
					System.out.println("Versuch Nr. " + trials + ": " + tip + " zu niedrig!");
				} else {
					System.out.println("Versuch Nr. " + trials + ": Sie haben die Zahl erraten!!!");
					guessed = true;
			...
			do {
				System.out.print("Wollen Sie das Spiel beenden (0), fortsetzen "
						+ "(1) oder den bisher besten Spieler(in) ausgeben " + "lassen (2)? ");
				end = scan.nextInt();
				if ((end < 0) || (end > 2))
					System.out.println("Eingabefehler!");
				if (end == 2) {
					System.out.println(
							"Bisher beste/r Spieler/in: " + bestPlayer + ", geringste Anzahl Versuche: " + leastTrials);
				}
			} while ((end < 0) || (end > 1));
		} while (end == 1);
	}
